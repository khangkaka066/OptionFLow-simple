...
function drawGexRibbon(ribbon, points, summary, session) {
  ...
  const dataTimes = [...times, ...(points || []).map(p => p.time).filter(Boolean)]
    .map(t => new Date(t))
  ...
  if (sessionOpenUtc && sessionCloseUtc && !Number.isNaN(sessionOpenUtc.getTime())) {
    ...
    const endUtc = new Date(Math.min(sessionCloseUtc.getTime(), Math.max(lastDataUtc.getTime(), minEndUtc.getTime())));
    layout.xaxis.range = [plotTimeNY(sessionOpenUtc), plotTimeNY(endUtc)];
    layout.xaxis.autorange = false;
    layout.xaxis.fixedrange = true;
    layout.xaxis.uirevision = "session-" + (session.trading_date || plotTimeNY(sessionOpenUtc).slice(0, 10)) + "-" + Math.floor(endUtc.getTime() / 60000);
  }
